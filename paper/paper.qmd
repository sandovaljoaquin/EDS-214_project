...
The environment.R script in the root directory contains all necessary packages for this analysis. Check to see if the packages are installed. If not, run the script.
...
combined_data <- readRDS(here::here("outputs", "combined_data.rds"))
```
...
```{r}
rolling_average <- readRDS(here::here("outputs", "rolling_average.rds"))
```
